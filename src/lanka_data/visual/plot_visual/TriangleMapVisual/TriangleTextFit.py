import math


class TriangleTextFit:
    ANGLES_DEG = (0.0, 60.0, -60.0)
    ADJACENCY_TOLERANCE = 0.5

    @staticmethod
    def _axes(angle_deg):
        theta = math.radians(angle_deg)
        cos_a, sin_a = math.cos(theta), math.sin(theta)
        return (cos_a, sin_a), (-sin_a, cos_a)

    @staticmethod
    def _project(point, axis):
        return point[0] * axis[0] + point[1] * axis[1]

    @classmethod
    def _lines(cls, points, u, v, line_spacing):
        lines = {}
        for point in points:
            key = round(cls._project(point, v) / line_spacing)
            lines.setdefault(key, []).append((cls._project(point, u), point))
        return lines

    @classmethod
    def _longest_in_line(cls, items, step):
        items.sort(key=lambda item: item[0])
        longest_run = run = [items[0]]
        tolerance = step * cls.ADJACENCY_TOLERANCE
        for prev, cur in zip(items, items[1:]):
            run = (
                run + [cur]
                if abs(cur[0] - prev[0] - step) < tolerance
                else [cur]
            )
            if len(run) > len(longest_run):
                longest_run = run
        return longest_run

    @classmethod
    def _best_run(cls, points, size):
        step = size
        line_spacing = size * math.sqrt(3) / 6
        best = None
        for angle_deg in cls.ANGLES_DEG:
            u, v = cls._axes(angle_deg)
            for items in cls._lines(points, u, v, line_spacing).values():
                run = cls._longest_in_line(items, step)
                if best is None or len(run) > best[0]:
                    best = (len(run), angle_deg, run)
        return best, step

    @staticmethod
    def _run_center(run):
        first, last = run[0][1], run[-1][1]
        return (first[0] + last[0]) / 2, (first[1] + last[1]) / 2

    @classmethod
    def best_label_fit(cls, points, size):
        (count, angle_deg, run), step = cls._best_run(points, size)
        cx, cy = cls._run_center(run)
        return cx, cy, count * step, size * math.sqrt(3) / 2, angle_deg
