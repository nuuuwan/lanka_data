...
class ColorSpecCustomMixin:
    ...
    def _get_custom_value_and_color(
        ...
        if has_non_float_values:
            color = cls.LABEL_TO_COLOR.get(
                custom_value
            ) or cls.p_to_color_for_category(p)
            return str(custom_value), color
        fmt = "{:+.4f}" if is_diff else "{:.4f}"
        value = (
            value_formatter(custom_value)
            if value_formatter
            else fmt.format(custom_value)
        )
        color = (
            cls.p_to_color_for_diff(p)
            if is_diff
            else cls.p_to_color_for_abs(p)
        )
        return value, color

    @staticmethod
    def _sort_key(value):
        parsed = Parse.float(value)
    ...
    def by_region_to_custom_value(
        ...
        sorted_custom_values = sorted(
            set(region_to_custom_value.values()), key=cls._sort_key
        )
        ...
        region_to_value, region_to_value_str = {}, {}
        for region_id, custom_value in region_to_custom_value.items():
            ...
            if not has_non_float_values:
                region_to_value[region_id] = float(custom_value)
                region_to_value_str[region_id] = value
        return cls(
            region_to_color,
            value_to_color,
            None,
            region_to_value or None,
            region_to_value_str or None,
        )
