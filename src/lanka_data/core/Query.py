...
class Query:
    ...
    def _make_what_info(
        self, prefix: str, domain: str, sub_parts: list[str]
    ) -> str:
        if sub_parts:
            return prefix + "-" + "-".join(sub_parts)
        default = self._DEFAULT_SUB.get(domain)
    ...
    def gig2_key(self) -> tuple[str | None, str | None]:
        ...
        prefix = self._DOMAIN_GIG2_PREFIX.get(parts[0])
        if prefix is None:
            return None, None
        sub_parts, sub_component = self._pop_sub_component(
            parts[1:], self._COMPOSITE_SUB_COMPONENTS
        )
        what_info = self._make_what_info(prefix, parts[0], sub_parts)
        return self.normalize(what_info), sub_component
