...
class WhatLabelValidator:
    ...
    def _check_every_label_has_dataset(self):
        ...
        for i_what_label, what_label in enumerate(
            unique_what_labels, start=1
        ):
            ...
            if matching_datasets:
                log.debug(
                    f"✅ {i_what_label}. '{what_label}' in {
                        ', '.join(
                            sorted(matching_datasets))}"
                )
            else:
        ...
        for dataset, labels in dataset_to_labels.items():
            for label in labels:
                ...
                if label not in valid_labels:
                    self.errors.append(
                        f"Dataset '{dataset}' has invalid label '{label}'"
                    )
                else:
